...
double Runge_Kutta(double h, double y, double t, double x, int n, double exact_func) {
    ...
    for(int i=0; i<n; i++){
        ...
        approx_k3 = h * approximation_function(t + (h/2.0), y + (approx_k2/2.0));
        approx_k4 = h * approximation_function(t + h, y + approx_k3);
        
        t = t + h;
        exact_func = exact_function(t);
        y = y + ((approx_k1 + 2.0*approx_k2 + 2.0*approx_k3 + approx_k4)/6.0);
    }
